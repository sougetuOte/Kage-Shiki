...
def main() -> None:
    ...
    mascot_view.set_persona_name(persona_core.c1_name)
    if persona_core.c3_second_person:
        mascot_view.set_user_display_name(persona_core.c3_second_person)

    # クリックイベント（突っつき）ハンドラ登録 (FR-2.5, T-14)
    def _on_poke(_x: int, _y: int) -> None:
        input_queue.put(
            f"{POKE_EVENT_PREFIX} ユーザーがウィンドウをクリックして突っつきました",
        )

    mascot_view.on_click(_on_poke)

    shutdown_cb = _make_shutdown_callback(memory_worker, db, shutdown_event)

    # ウィザード再起動コールバック（system_tray はクロージャで後から参照）
    _tray_ref: list[SystemTray | None] = [None]

    def _wizard_restart() -> None:
        """persona ファイルをバックアップしてウィザードモードで再起動する."""
        persona_path = data_dir / "persona_core.md"
        if persona_path.exists():
            backup = persona_path.with_suffix(".md.bak")
            shutil.copy2(persona_path, backup)
            persona_path.unlink()
            logger.info("persona_core.md を %s にバックアップしました", backup)
        # トレイアイコン停止 + シャットダウン + 新プロセス起動
