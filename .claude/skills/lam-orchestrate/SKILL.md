...
### lam-loop-state.json スキーマ

自動ループの状態管理ファイル（`.claude/lam-loop-state.json`）。`/full-review` の自動イテレーションで使用する。

> **注**: `fullscan_pending` フィールドは廃止済み（常にフルスキャンのため未使用）。
> `green_state.G3_issues` / `G4_spec_sync` は `/full-review` Phase 4 で設定される（stop hook は G1/G2/G5 のみ自動評価）。
...
1. **初期化**: `/full-review` 開始時に `lam-loop-state.json` を作成
2. **状態更新**: 各 Phase 完了時に対応フィールドを更新
3. **判定**: Green State 5条件を評価し、未達なら iteration をインクリメント
4. **終了処理**: Green State 達成または max_iterations 到達で終了、ファイルを削除

### hooks 連携

- **PostToolUse hook**: テスト実行後に TDD パターンを記録
- **lam-stop-hook**: Green State 判定（G1/G2/G5 のみ自動評価）を行い、未達なら `/full-review` を再トリガー
- hooks 未導入時は手動フォールバック（ユーザーに再実行を提案）

> **G3（対応可能 Issue ゼロ）** と **G4（仕様差分ゼロ）** は `/full-review` Phase 4 で
> Claude が評価する。stop hook は G1（テスト）、G2（lint）、G5（セキュリティ）のみ自動判定する。
> `green_state` オブジェクトの `G3_issues` / `G4_spec_sync` フィールドは
> `/full-review` Phase 2 のレポート統合時に設定される。

### エスカレーション条件

以下の場合、自動ループを停止しユーザーに報告する:
- PM級 Issue が新規検出された
- 同一 Issue が 2 イテレーション連続で修正できない
- max_iterations に到達
