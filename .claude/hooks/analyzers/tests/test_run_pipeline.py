...
class TestRunPhase0:
    ...
    _GL = patch("analyzers.run_pipeline.gitleaks_run_detect", return_value=[])
    # bandit が未インストールの環境でも verify_tools を通過させるモック
    _WH = patch("analyzers.base.shutil.which", return_value="/usr/bin/mock")

    def _mocks(self, ruff_out: str = "[]"):
        """subprocess + gitleaks + which のモックをまとめて返す。"""
        return (
            patch("subprocess.run", side_effect=_subprocess_side_effect(ruff_out)),
            self._GL,
            self._WH,
        )
    ...
    def test_tool_not_found_raises(self, project_root: Path) -> None:
        ...
        (project_root / "app.py").write_text("x = 1\n")
        with (
            patch("analyzers.base.shutil.which", return_value=None),
            self._GL,
...
class TestRunPhase0GitleaksIntegration:
    ...
    対応仕様: docs/specs/lam/gitleaks-integration-spec.md FR-1
    対応設計: docs/design/gitleaks-integration-design.md Section 5.4
    """
    ...
    def test_run_phase0_includes_gitleaks(self, project_root: Path) -> None:
        ...
        with (
            ...
            patch("analyzers.base.shutil.which", return_value="/usr/bin/mock"),
        ):
            result = run_phase0(project_root)
