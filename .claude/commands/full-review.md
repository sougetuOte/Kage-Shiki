...
> **⚠️ Phase 3（Hooks + analyzers/）完了まで /full-review を実行しないこと（stop-hook 二重判定リスク）**
...
### Step 3: Scale Detection 判定

プロジェクト規模に応じて有効化する Plan セットを自動判定する。

**Scale Detection テーブル（Plan A-E 閾値）**:

| Plan | LOC 目安 | 概要 |
|:-----|:---------|:-----|
| なし | ~10K | 従来モード（チャンクなし、トポロジカルなし） |
| A | ~10K+ | 静的解析パイプライン（ruff/bandit + AST/import-map） |
| A + B | ~30K | チャンク分割レビュー（tree-sitter AST チャンキング） |
| A + B + C | ~50K | 階層的レビュー（Layer 2 モジュール統合 + Layer 3 システムレビュー） |
| A + B + C + D | ~100K+ | トポロジカル順レビュー・修正 + 契約カード注入 |

> **影式の現状**: ~5K LOC → Plan E（~10K）tier に相当。Plan なし（従来モード）。
> Stage 1 の静的解析パイプラインは Phase 3（analyzers/ 導入）完了後に Plan A として有効化される。

```bash
python .claude/hooks/analyzers/scale_detector.py "$TARGET"
```
...
> **注**: 影式では analyzers/ が未導入のため、Phase 3 完了まで Stage 1 はスキップされる。
...
| `quality-auditor` | アーキテクチャ・仕様整合性（依存関係、**仕様ドリフト**、**構造整合性**）+ **`building-checklist.md` R-1〜R-13, S-1〜S-2 適合性** | 仕様ドリフト + 構造整合性結果を含む |
| `code-reviewer` (3) | セキュリティ（OWASP Top 10、シークレット漏洩、依存脆弱性、インジェクション） | 各 Issue にリスクレベル (Critical/High/Medium/Low) + PG/SE/PM 分類を付与 |

> **影式固有**: quality-auditor は `.claude/rules/building-checklist.md` の全ルール（R-1〜R-13, S-1〜S-2）への適合性を検査すること。LAM コアルール（R-1, R-4, S-1, S-3, S-4）に加え、影式固有ルール（R-2, R-3, R-7〜R-13, S-2）も対象。
...
| Layer | 名称 | 観点 | 実行条件 |
|:------|:-----|:-----|:---------|
| Layer 1 | ファイルレビュー | 個別ファイルの品質（Stage 2 で実施済み） | 常に |
| Layer 2 | モジュール統合 | モジュール境界、要約カード、契約カード永続化 | Plan C 以上 |
| Layer 3 | システムレビュー | 循環依存、命名規則、仕様ドリフト（機械的 + LLM） | Plan C 以上 |
...
```bash
# PM級 Issue 発見時に実行
python -c "import json,pathlib;p=pathlib.Path('.claude/lam-loop-state.json');d=json.loads(p.read_text());d['pm_pending']=True;p.write_text(json.dumps(d,indent=2,ensure_ascii=False))"
```

```bash
# PM級修正完了後に実行
python -c "import json,pathlib;p=pathlib.Path('.claude/lam-loop-state.json');d=json.loads(p.read_text());d['pm_pending']=False;p.write_text(json.dumps(d,indent=2,ensure_ascii=False))"
```
...
> **影式固有**: G1 の検証には `pytest tests/ -v --tb=short` を使用し、G2 には `ruff check src/ tests/` を使用すること。

#### 真の Green State（Before=0 再スキャン要件）
...
| シークレット漏洩 | **gitleaks**（Stage 1 Step 1 で実行済み） | gitleaks Issue ゼロ（`gitleaks:not-installed` 含む） |
| 危険パターン | OWASP Top 10 チェック | eval/exec、SQL文字列結合、pickle.load 等なし |

**gitleaks 未インストール時の G5 判定**: `gitleaks:not-installed` Issue は Critical として扱われるため、**G5 は FAIL** となる。gitleaks をインストールして再実行すれば解消される。インストールガイドは Stage 1 Step 1 のログに表示される。
...
**fullscan_pending の発動手順**: 差分チェックで Green State を達成したら、Claude が状態ファイルに `fullscan_pending: true` をセットし、自分で Stage 2 に戻る:

```bash
# 差分チェック Green State 達成時に実行
python -c "import json,pathlib;p=pathlib.Path('.claude/lam-loop-state.json');d=json.loads(p.read_text());d['fullscan_pending']=True;p.write_text(json.dumps(d,indent=2,ensure_ascii=False))"
```
...
- 要件仕様: `docs/specs/lam/scalable-code-review-spec.md`
