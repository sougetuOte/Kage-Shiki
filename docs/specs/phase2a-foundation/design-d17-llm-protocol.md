...
> **Wave 2 Full Review による決定更新**: 上記の Mediator 結論は当初の設計意図を記録した履歴として残す。Wave 2 Full Review（commit 0216817）において、FR-8.11（統合テスト要件）の分析から `send_message_for_purpose()` を `LLMProtocol` に追加することが決定された。最終的な実装は `chat()` と `send_message_for_purpose()` の2メソッドを Protocol に定義している。詳細は Section 4 および Section 5.1 を参照。
...
**統合結論**（Wave 2 Full Review により更新）:
- Protocol メソッドは `chat()` と `send_message_for_purpose()` の2つ
...
- `send_message_for_purpose()` は Wave 2 Full Review（commit 0216817）で `LLMProtocol` に追加された（FR-8.11 統合テスト要件による）

---

## 4. 決定

**採用**: `LLMProtocol` を `typing.Protocol` として `llm_client.py` に追加。`LLMClient` は Protocol を満足するように `chat()` および `send_message_for_purpose()` メソッドを持つ。

**理由**:
- `chat()` でプリミティブな LLM 送受信を抽象化する（当初設計）
- `send_message_for_purpose()` を Wave 2 Full Review（commit 0216817）で Protocol に追加した。FR-8.11 の統合テストで `AgentCore` が `LLMProtocol` 型経由で `send_message_for_purpose()` を呼び出す要件が明確化されたため
- C-3（内部実装不変）と整合する
- Phase 3 でのスムーズな LocalLLM 追加を可能にする（第2実装は `send_message_for_purpose()` も実装する必要がある）
...
class LLMProtocol(Protocol):
    ...
    Methods:
        ...
        send_message_for_purpose: 用途に応じたパラメータで LLM を呼び出す。
            （Wave 2 Full Review で Protocol に追加、FR-8.11 統合テスト要件による）
    """
    ...
    def send_message_for_purpose(
        ...
        Wave 2 Full Review（commit 0216817）で Protocol に追加。
        FR-8.11 の統合テストにより、AgentCore が LLMProtocol 型経由で
        この メソッドを呼び出す要件が明確化されたため Protocol に昇格。
...
**Wave 2 での Protocol 昇格（S-1/S-4 同期）**:
`send_message_for_purpose()` は当初 `LLMClient` 固有の Protocol 外メソッドとして設計されたが、
Wave 2 Full Review（commit 0216817）において `LLMProtocol` に追加された。
FR-8.11 の統合テスト要件から、`AgentCore` が `LLMProtocol` 型変数経由でこのメソッドを呼び出す
必要があることが確認されたため、Protocol への昇格を決定した。

これにより、静的型チェッカーは `LLMProtocol` 型から `send_message_for_purpose()` を呼び出す箇所を
エラーとして報告しなくなった。Phase 3 で LocalLLM を追加する際は、この実装も `send_message_for_purpose()`
を実装する必要がある（purpose 体系の対応が課題となる）。
...
│   ├── chat()
│   └── send_message_for_purpose()（Wave 2 Full Review で Protocol に追加）
└── LLMClient（変更なし + chat() メソッド追加）
    ├── send_message()（変更なし）
    ├── send_message_for_purpose()（変更なし — Protocol メソッドとして機能）
...
class MockLLMClient:
    ...
    chat() と send_message_for_purpose() の両方を実装する。
    Wave 2 Full Review により send_message_for_purpose() が LLMProtocol に
    追加されたため、完全な Protocol 実装としてどちらも提供する。
    """
...
> **注意**: Wave 2 Full Review により `send_message_for_purpose()` が `LLMProtocol` に追加されたため、`MockLLMClient` も両メソッドを実装する。単体テスト（`chat()` のみを検証したい場合）は `send_message_for_purpose()` をスタブとして残すだけで十分（FR-8.11 統合テスト参照）。
...
| 決定事項 | 内容 |
|---------|------|
| `send_message_for_purpose()` の Protocol 追加（Wave 2 で決定） | 当初は Protocol 外メソッドとして設計したが、FR-8.11 統合テスト要件により Wave 2 Full Review（commit 0216817）で Protocol に昇格。Phase 3 で LocalLLM を追加する際は purpose 体系の対応が課題 |
| `LLMClient` 改名の保留 | Phase 3 で `AnthropicClient` への改名を検討 |
| LocalLLM での purpose 体系対応 | `send_message_for_purpose()` が `AppConfig.VALID_PURPOSES` 等 Anthropic 固有の概念に依存しているため、Phase 3 の LocalLLM 実装時にアダプタ設計を検討する |
