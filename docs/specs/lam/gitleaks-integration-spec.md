...
### NFR-1a: line_count の算出

`run_phase0()` が返す `line_count` は言語 Analyzer が検出したファイルの行数合計である。
言語 Analyzer が 1 つも検出されない場合（対象ディレクトリに `.py`/`.js`/`.rs` 等がない場合）、
`line_count` は 0 となる。gitleaks は言語 Analyzer に依存せず実行されるが、
`line_count` は Scale Detection（Plan A-E 閾値判定）に使用されるため、
解析対象言語のファイルがない環境では 0 が意味的に正しい。
...
- 既存仕様: `docs/specs/lam/scalable-code-review-spec.md` FR-7e
