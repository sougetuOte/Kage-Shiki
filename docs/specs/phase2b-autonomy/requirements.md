...
DesireWorker は `asyncio` ループ内で定期実行されるワーカーで、欲求レベルの更新と閾値超過通知を担当する。
...
### 5.2 AgenticSearchEngine Protocol（新規）
    ...
    +-- summarize(results: list[SearchResult]) -> str  # 検索結果の要約（LLM）
...
Phase 1 の通信モデルに以下の経路を追加する:
    ...
    DesireWorker（asyncio ループ内）
        -> コールバック -> AgentCore.handle_autonomous_turn()
            -> queue.Queue -> MascotView.schedule() -> GUI 表示
...
3. **asyncio と tkinter の共存（対応）**: D-26 で既存の threading + queue.Queue モデルとの整合性を確保する。DesireWorker は asyncio ループ内のタスクとして実装し、AgentCore の既存 asyncio ループに統合する。新たなスレッドは追加しない方向で設計
