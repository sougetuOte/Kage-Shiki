...
| 関数 | 説明 |
|------|------|
| `register_windows_ctrl_handler(shutdown_callback) -> bool` | SetConsoleCtrlHandler を登録する。成功なら True、失敗なら False + WARNING ログ |
| `make_atexit_handler(shutdown_callback) -> Callable` | atexit 用ラッパーを生成する。`_shutdown_done` フラグで2重実行を防止 |
| `reset_shutdown_state() -> None` | テスト用: `_shutdown_done` フラグと `_ctrl_handler_ref` をリセットする（本番コードからは呼ばない） |

`_make_ctrl_handler` は内部関数（Protocol 外メソッド）。テストでは `shutdown_handler._make_ctrl_handler` を直接 import して利用する。
