...
- 検証結果は `docs/memos/walkthrough-<feature>.md` にまとめ、スクリーンショットやログと共に報告することを必須とする。

### Automated TDD Introspection（v4.0.0）

PostToolUse hook がテスト失敗→成功パターンを自動記録する:

1. テスト失敗時: `.claude/tdd-patterns.log` に FAIL エントリ追記
2. テスト成功（前回失敗）: PASS (previously failed) エントリ追記
3. 同一パターン 3回到達: `.claude/rules/auto-generated/draft-*.md` にルール候補を生成
